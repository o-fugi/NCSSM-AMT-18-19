...
from const_city_size import power_percent_dem 
from const_city_size import return_width 
...
# For keeping track of max_seats
max_seats = []
seats_won_arrays = []
...
# The cities are most Democratic in the center and decrease by 1/distance**power in percent Democratic
def makeCityDistribution(iterating_factor, value, city_locations):
    ...
    # If the number or position of cities is being modified, change here
    ...
    for y, row in enumerate(percent_dem):
        for x, column in enumerate(row):
            if map_type == "voronoi" or map_type == "random_generate":
                factor = max(distFromCityNoWrap(y, x, city_locations, prec_dim_x, prec_dim_y), 1)
            else:
                factor = max(distFromCityWrap(y, x, city_locations, prec_dim_x, prec_dim_y), 1)
            if city_type=="power":
                row[x] = power_percent_dem(factor, width, intensity)
            else:
                row[x] = math.exp(-intensity*factor)
...
def makeStateIntoNodes(state, numDists):
    ...
    for row in range(state.shape[0]):
        for col in range(state.shape[1]):
            ...
            nextNode.population = ((state[row][col]  - 0.5)**5 + 0.5) * (totalPop * 2 / (state.shape[0] * state.shape[1]))
...
def analysisExample():
    ...
    for value in iterating_factor.iterating_range:
        ...
        distr_medians = np.asarray(distr_medians)
        ...
        # try new method of electoral responsiveness??
        distance_from_center = np.mean(abs(distr_medians - 0.5))
    ...
    ax1.set_title("Boxplot Distances from 50%")
    ax1.set_xlabel(iterating_factor.name)
    ax1.set_xticklabels([])
    ax1.set_ylabel("Percent Democratic")
    # ax1.plot(iterating_factor.iterating_range, [i - j for (i, j) in zip(average_slopes, slopes_at_50)])
    ax1.plot(iterating_factor.iterating_range, distances)

    ax2 = plt.subplot(2, 1, 2)
    ax2.set_title("Difference between Actual and Expected Seats Won")
    ax2.set_xlabel(iterating_factor.name)
    ax2.set_xticklabels([])
    ax2.set_ylabel("Seat Advantage")
    print(seats_won)
